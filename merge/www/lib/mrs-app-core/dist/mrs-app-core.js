/*! mrs-app-core - v1.1.5 - 2015-12-01 10:37:25 GMT */
/**
        ...
        boot = function loadAngAppBoot(moduleName, configFile) {
            ...
            function onLoadConfigFileSuccess(data) {
                ...
                try {
                    ...
                    console.error("FATAL: ERROR BOOTSTRAP", e);
                }
...
/**
    ...
    @since 0.1.0
**/
angular.module("MRS.App.Core").service("dataConnector", ["$injector", "$log", "$q", function mrsCoreDataConnector($di, $log, $q) {
    'use strict';
...
**/
    ...
    /**
        ...
        @method getPolicies
        @return {Object} Policies definitions.
    **/
    ...
    /**
     * Get policy by name. Default is ONLINE.
     * 
     * @method getPolicy
     * @public
     * @param {string} policyName
     * @return {object} policy structure
     */
    this.getPolicy = function dataConnectorGetPolicy(policyName) {
        return self.getPolicies()[policyName || "ONLINE"];
    };
    ...
    **/
    this.fetch = function dataConnectorFetch(request, policy) {
        ...
        try {
            // check policy
            // if policy is undefined, try to resolve by request.policy
            policy = policy || self.getPolicy(request.policy);
            if (!angular.isDefined(policy)) {
                return $q.reject('No valid policy defined');
            }
...
angular.module('MRS.App.Core').service('webProxy', ['$http', '$q', 'webCache', function ($http, $q, webCache) {
    ...
    function interpolate(string, params) {
        var result = [];
        angular.forEach((string || '').split(interpolateChar), function (segment, i) {
            if (i === 0) {
                result.push(segment);
            } else {
    ...
    **/
    ...
    /**
        ...
        @param timestamp {Number} An optional timestamp number in milisseconds to consider cache dirty.
    **/
    self.hit = function (options, cache, forceHit, timestamp) {
        var salt = (new Date()).getTime().toString() + Math.random().toString(),
            promiseExists = false,
            cancelPromiseName,
            defer,
            i;
        ...
        // check if we must hit the server
        var mustHitServer = (!cache ? true : (webCache.get(options, timestamp) === null));
        ...
        // returning promise
        return webCache.defer(options, cache);
    };
    ...
    **/
    self.send = function (request) {
        ...
        if (!angular.isDefined(request.url)) {
            return $q.reject('No endpoint defined');
        }
        
        // check method
        if (!angular.isDefined(request.method)) {
            return $q.reject('No method defined');
        }
        ...
        // handle baseUrl
        if (request.baseUrl !== undefined && options.url.indexOf('://') === -1) {
        ...
        // handle url and parse/format using data
        options.url = interpolate(options.url, options.data);
...
/**
 * The 'MRS.App.Helper' module provide some utilities to build other components
 * such as AngularJS services and factories.
 * 
 * It as some conventions according to MRS.Server (v5) and MRS.App.Core compatibility.
 * 
 * @module MRS.App.Core
**/
angular.module('MRS.App.Helper', ['MRS.App.Core']);

// Providers configuration
angular.module('MRS.App.Helper').config(['$mrsapphelperConfig', function (config) {
    'use strict';
    var mergedConfig,
        defaultConfig = {};
...
**/
angular.module('MRS.App.Helper').factory('MRSAppHelper.Service', ['$log', '$q', 'dataConnector', function ($log, $q, $connector) {
    "use strict";

    var defaultAdapterMethod = {
        
        to: function defaultToAdpterMethod(request) {
            return request;
        },
        
        from: function defaultFromAdapterMethod(result) {
    ...
    /**
     * Fetch request to data connector.
     * It uses default adapter if none is provided
     * 
     * @method fetch
     * @private
     * @param {object} request The request object to fetch data
     */
    function fetch(request) {
        // check for request adapter or add default
        request.adapter = angular.extend({}, defaultAdapterMethod, request.adapter);
        
        return $connector
            .fetch(request.adapter.to(request))
            .then(request.adapter.from);
    };
    
    
    /**
     * Create a list of services associated with given method.
     * 
     * @method serviceBuilder
     * @param {Function} createServiceMethodFunc The method that will be associated with given list of services
     * @param {String[]} listOfServiceMethods List of service names to create
     * @private
     * @return {Object} Object with services pointing to given method
     */
    function serviceBuilder(createServiceMethodFunc, listOfServiceMethods) {
        var key,
            result = {},
            serviceMethod;
            
        for(key in listOfServiceMethods) {
            serviceMethod = listOfServiceMethods[key];
            result[serviceMethod] = createServiceMethodFunc(serviceMethod);
        }
    ...
    /**
     * @method fetchWithDefaultConvention
     * @private
     */
    function fetchWithDefaultConvention(adapterInstance, configInstance, serviceMethodName, data) {
        // build request using default config
        // method config overrides default config; default config overrides local default config
        
        // we must parse data in separate loop because data has inside members
        // and angular.extend isn't recursive! angular.merge is only available in v4.x 
        var requestData = angular.extend(
            {},
            (configInstance['_default'] && configInstance['_default'].data) || {},
            configInstance[serviceMethodName].data || {},
            data
        );
        
        var request = angular.extend(
            {},
            configInstance['_default'] || {},
            configInstance[serviceMethodName],
            {
                adapter: adapterInstance[serviceMethodName],
                data: requestData
            }
        );
    ...
    /**
     * Creates new services with default request conventions.
     * It uses default method and fetch server with default convention.
     * 
     * @method serviceBuilderWithDefaultConvention
     * @private
     * @param {Object} adapter The adapter object containing all methods adapter
     * @param {Object} config The config object containing all methods adapter
     * 
     */
    function serviceBuilderWithDefaultConvention(adapter, config, listOfServiceMethods) {
        
        var defaultMethod = function defaultMethod(serviceMethodName) {
            return function (data) { 
                return fetchWithDefaultConvention(adapter, config, serviceMethodName, data);
            };
        };
            
        return serviceBuilder(defaultMethod, listOfServiceMethods);
    }
    ...
    return {
         ...
         * @public
         */
        serviceBuilderWithDefaultConvention: serviceBuilderWithDefaultConvention
    };
